...
(defn- rewrite-grouping-sets-sql
  ...
  (let [upper    (str/upper-case sql)
    ...
    (when-not from-idx
      (log/errorf "pivot-gs: no FROM found. SQL starts with: %s" (subs sql 0 (min 200 (count sql)))))
...
(defn- run-pivot-grouping-sets
  ...
  (let [sql           (:query compiled)
    ...
    (when-not group-cols
      (log/errorf "pivot-gs: no GROUP BY found in SQL (%d chars), falling back" (count sql)))
    (when group-cols
      (let [num-breakouts (count (lib/breakouts query))
        ...
        (when gs-sql
          (log/infof "pivot-gs: %d breakouts, %d combos, %d group-cols, SQL %d chars"
                     num-breakouts (count combos) (count group-cols) (count gs-sql))
   ...
   (qp.setup/with-qp-setup [query query]
     (let [query       (qp.middleware.normalize/normalize-preprocessing-middleware query) ; normalize to MBQL 5 if needed.
           ...
           compiled    (when use-gs?
                         (try
                           (let [c (qp.compile/compile query)]
                             (log/infof "pivot-gs: compile OK, keys=%s query-type=%s"
                                        (pr-str (keys c)) (type (:query c)))
                             c)
                           (catch Throwable t
                             (log/errorf t "pivot-gs: compile FAILED")
                             nil)))
